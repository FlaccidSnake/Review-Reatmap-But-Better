...
/* Heatmap activity type widget (DROPDOWN FIXES APPLIED HERE) */
.hm-sel {
	display: inline-block;
	height: 100%;
	padding: 4px 8px;
	/* font-size: 80%;  <-- REMOVED THIS SO IT MATCHES BUTTONS */
	cursor: pointer;
	color: #808080;
	border-radius: 3px;
	user-select: none;
...
select.hm-sel:active,
select.hm-sel:focus {
	background: url(qrc:/review_heatmap/icons/down.svg) 96% / 10% no-repeat #e6e6e6;
}

.night_mode select.hm-sel:active,
.night_mode select.hm-sel:focus {
	background: url(qrc:/review_heatmap/icons/down.svg) 96% / 10% no-repeat #313d45;
    color: #aeb0b2;
}
...
(function(factory) {
  ...
  Legend.prototype.getClass = function(n, withCssClass) {
    ...
    // Check if it's a forecast (negative number)
    if (this.calendar.options.legend[0] > 0 && n < 0) {
        // Use index 1 (Gray/Forecast color)
        // You can change "1" to a different number (1-10) if you want a different shade of gray
        index = ["1", "i"]; 
    } 
    else {
        // It's history (positive number)
        for (var i = 0, total = this.calendar.options.legend.length - 1; i <= total; i++) {
            if (n <= this.calendar.options.legend[i]) {
                // Shift by 11 to use the Colored Theme classes (q11-q20)
                index = [i + 11]; 
                break;
            }
        }
  ...
  // Modified sections of anki-review-heatmap.js
...
class ReviewHeatmap {
  ...
  create(data) {
    ...
    heatmap.init({
      ...
      itemName: this.getItemName(activityType),
      ...
      subDomainTitleFormat: (isEmpty, formatData, cellData) => {
        return this.formatTooltip(isEmpty, formatData, cellData, activityType);
      },
      onClick: (date, nb) => {
        this.handleClick(date, nb, activityType, heatmap, calTodayDate);
      },
  ...
  formatTooltip(isEmpty, formatData, cellData, activityType) {
    let tooltip;
    ...
    if (isEmpty) {
      let emptyText = this.getEmptyText(activityType, cellData.t);
      tooltip = `<b>No</b> ${emptyText} on ${formatData.date}`;
    } else {
      const label = Math.abs(cellData.v) == 1 ? "card" : "cards";
      const action = this.getActionText(activityType, cellData.v);
      tooltip = `<b>${count}</b> ${label} <b>${action}</b> ${formatData.connector} ${formatData.date}`;
    }
    return tooltip;
  }
